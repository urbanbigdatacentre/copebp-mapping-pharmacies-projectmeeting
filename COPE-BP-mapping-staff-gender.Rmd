...
```{r db_select_pharmacies_staff_sex, include=FALSE}
...
```{r db_count_pharmacies_staff_sex, include=FALSE}
...
```{r db_cat_pharmacies_sex_clients, include=FALSE}
bd_pharm_staff_sex <- bd_pharm_staff_sex %>% 
  mutate(female_staff = case_when(
    count_female >= 1 ~ 'YES',
    TRUE ~ 'NO'
  ))
...
```{r save_data_as_geojson, include=FALSE}
st_write(bd_pharm_staff_sex, "survey-data/bd_pharm_staff_sex.geojson", delete_dsn = TRUE)
st_write(pk_pharm_staff_sex, "survey-data/pk_pharm_staff_sex.geojson", delete_dsn = TRUE)
```



```{r map_pharm_staff_sex, echo=FALSE}
...
color_loc <- colorFactor(
  ...
  domain = bd_pharm_staff_sex$female_staff
)
...
leaflet() %>%
  ...
  addCircleMarkers(
    data = bd_pharm_staff_sex,
    color = ~color_loc(female_staff),
    radius = 2.8,
    fillOpacity = 0.6,
    popup = ~paste("_id: ", as.character('_id')),
    group = "Pharmacies - Staf by SEX"
  ) %>% 
  addCircleMarkers(
    data = pk_pharm_staff_sex,
    color = ~color_loc(female_staff),
    radius = 2.8,
    fillOpacity = 0.6,
    popup = ~paste("_id: ", as.character('_id')),
  ...
  addLegend(
    "bottomright",
    pal = color_loc,
    values = pk_pharm_staff_sex$female_staff,
    title = "Female stuff in Phramacy"
  ) %>% 
  # Add layers control (toggle and radio)
  addLayersControl(
    baseGroups = c("OpenStreetMap", "Satellite"),
    overlayGroups = c("Pharmacies - Staf by SEX"),
