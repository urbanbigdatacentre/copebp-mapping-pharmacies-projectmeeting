...
```{r bg_geo_uc_filter, include=FALSE}
# For Bangladesh, at the moment, geospatial data is not integrated due to some missing values for location
bd_wards <- st_read("Reference-data/bgd_admbnda_adm4_bbs_20201113.geojson")
bd_wards <- bd_wards %>% 
  filter(!is.na(survey_code)) %>% 
  select(ADM4_EN, ADM2_EN, survey_code, geometry)

```

```{r bg_geo_function_curves, include=FALSE}
...
bd_pharm_multi_area$client_area <- as.integer(substr(bd_pharm_multi_area$catch_area, nchar(bd_pharm_multi_area$catch_area) - 2 + 1, nchar(bd_pharm_multi_area$catch_area)))



```

Note: For the Bangladesh analysis, we only obtained data about Dhaka wards. Analysis of areas in Narsingdi is pending
```{r bg_geo_build_curves, warning=FALSE, include=FALSE}

# Remove rows where the client area is not identified
bd_pharm_areas <- bd_pharm_multi_area %>% 
  filter(client_area < 30)

bd_pharm_areas_arcs <- st_sf(pharmid = character(),
  survey_code = character(),
  geometry = st_sfc(crs = 4326))

for(i in 1:nrow(bd_pharm_areas)) {
  # Get value of pacient area
  areacode <- bd_pharm_areas[i,]$client_area
  
  # Find the referred ward
  match_ward <- bd_wards %>% 
    filter(survey_code == areacode)
  
  new_row <- st_sf(
    pharmid = bd_pharm_areas[i,]$pharmid,
    survey_code = match_ward$survey_code,
    geometry = st_sfc(create_arc(bd_pharm_areas[i,], st_centroid(match_ward), 50), crs = 4326)
  )

  bd_pharm_areas_arcs <- rbind(bd_pharm_areas_arcs, new_row)
}
...
```{r bg_save_geospatial_data, include=FALSE}
...
```{r bg_export_all_pharmacies, include=FALSE}
bd_all_pharmacies_layer <- bd_data_geo %>% 
  select ("_id",  "pq_district", "pq_city", "pq_ward", "pharmacy_id", "pq_screen", "pq_ns_res", "pq_pharm_catg",
          ...
          "pq_scr_c1", "pq_scr_d", "pq_non_eligible", "pq_eligible", "pharmacy_inclusion", "pq_consent", "pq_c_daily_cus")
st_write(bd_all_pharmacies_layer, "survey-data/bd-all-pharmacies-geo.geojson", driver = "GeoJSON", delete_dsn = TRUE)
...
```{r pk_save_geospatial_data, include=FALSE}
...
```{r pk_export_all_pharmacies, include=FALSE}
pk_all_pharmacies_layer <- pk_data_geo %>% 
  select ("_id",  "pq_district", "pq_city", "pq_ward", "pq_calc", "pharmacy_id", "pq_screen", "pq_ns_res", "pq_pharm_catg",
          "pq_ns_visit_1", "pq_ns_visit_2", "pq_ns_type", "pq_ns_res_oth", "pq_scr_a", "pq_scr_b", "pq_scr_c",
          "pq_scr_c1", "pq_scr_d", "pq_non_eligible", "pq_eligible", "pharmacy_inclusion", "pq_c_daily_cus")
st_write(pk_all_pharmacies_layer, "survey-data/pk-all-pharmacies-geo.geojson", driver = "GeoJSON", delete_dsn = TRUE)
```